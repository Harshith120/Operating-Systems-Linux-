...
#include <fs_ops.h> // Angle brackets are for system headers
...
bool FSManage::format_fs(int file_names, int DABPT_entries)
{
    try
    {
        current->format(file_names, DABPT_entries);
    }
...
void Disk::put(std::string name)
{
    ...
    auto temp = new char{FS::ENTRY_MAX};
    ...
    while (!disk.eof())
    {
        ...
        for (; curr < free_space.size(); curr++)
        { //  curr for a slot that is free
...
bool FSManage::rename(std::string old_file_name, std::string new_file_name)
{
    try
    {
        current->rename(old_file_name, new_file_name);
    }
    catch (const std::exception &e)
    {
        std::cerr << e.what() << std::endl;
...
void Disk::user(std::string file_name)
{
    ...
    auto i{0};
    for (; i < new_name.size() &&
           i < (FS::BLOCK_SIZE / 4 - sizeof(int) * 4 - 1);
         i++)
